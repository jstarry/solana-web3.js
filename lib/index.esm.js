...
function () {
  ...
  _createClass(Transaction, [{
    ...
    value: function _getSignData() {
      ...
      var signDataLayout = struct([blob(1, 'numRequiredSignatures'), blob(1, 'numCreditOnlySignedAccounts'), blob(1, 'numCreditOnlyUnsignedAccounts'), blob(keyCount.length, 'keyCount'), seq(publicKey('key'), keys.length, 'keys'), publicKey('recentBlockhash')]);
      var transaction = {
...
var GetBalanceRpcResult = struct$1({
...
var ProgramAccountInfoResult = struct$1(['string', AccountInfoResult]);
/***
...
var GetSignatureStatusRpcResult = jsonRpcResult(struct$1.union(['null', struct$1.union([struct$1({
  Ok: 'null'
}), struct$1({
  Err: 'object'
})])]));
...
var GetRecentBlockhash = jsonRpcResult(['string', struct$1({
...
function () {
  ...
  _createClass(SystemProgram, null, [{
    ...
    value: function createAccount(from, newAccount, lamports, space, programId) {
      var dataLayout = struct([u32('instruction'), ns64('lamports'), ns64('space'), publicKey('programId')]);
    ...
    value: function assign(from, programId) {
      var dataLayout = struct([u32('instruction'), publicKey('programId')]);
      var data = Buffer.alloc(dataLayout.span);
      dataLayout.encode({
        instruction: 1,
        // Assign instruction
 ...
 * @private
 */
var TokenInfoLayout = struct([uint64('supply'), u8('decimals'), rustString('name'), rustString('symbol')]);
/**
 ...
 * @private
 */
var TokenAccountInfoLayout = struct([publicKey('token'), publicKey('owner'), uint64('amount'), u8('sourceOption'), publicKey('source'), uint64('originalAmount')]);
// This type exists to workaround an esdoc parse error
...
export { Account, BpfLoader, BudgetProgram, Connection, Loader, NativeLoader, PublicKey, SystemProgram, Token, TokenAmount, Transaction, TransactionInstruction, sendAndConfirmRawTransaction, sendAndConfirmTransaction, testnetChannelEndpoint };
